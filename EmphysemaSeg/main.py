...
def infer(config, images, emp_seg=False, vis=False):
    print(emp_seg, vis)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    # Create output directories if they don't exist
    os.makedirs(config["seg_dir"], exist_ok=True)

    # Use the provided file list instead of glob
    ...
    with torch.no_grad():
        for batch in tqdm(infer_loader):
            ...
            nib.save(label_map_nii, os.path.join(config["seg_dir"], f"{fname}_lobes.nii.gz"))

            if vis:
                os.makedirs(config["clip_dir"], exist_ok=True)
                if emp_seg:
                    # label map encodes emphysema as 7
...
def infer_dataloader(config, image_files):
...
def main():
        ...
        parser.add_argument('--file_list', type=str, help='Path to a text file containing a list of files (one per line)')
        ...
        if args.file_list:
                # Read file paths from the provided list file
                with open(args.file_list, 'r') as f:
                        image_files = [line.strip() for line in f if line.strip()]
                print(f"Loaded {len(image_files)} files from list file")
        else:
                # Fall back to using data_dir from config
                data_dir = config['data_dir']
                image_files = glob.glob(os.path.join(data_dir, "*.nii.gz"))
                print(f"Found {len(image_files)} files in {data_dir}")
        
        infer(config, image_files, emp_seg=args.emp, vis=args.vis)
